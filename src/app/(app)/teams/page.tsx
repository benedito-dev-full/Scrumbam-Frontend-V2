      ...
      {[
        ...
        <div key={i} style={{ position: "relative", height: 18, marginBottom: 8 }}>
          <div style={{
            ...
            height: 14, borderRadius: 4, background: b.color, opacity: 0.85, top: 2,
          }} />
        </div>
      ))}
      {/* Members section */}
        ...
        <div style={{ display: "flex", gap: -4 }}>
          {["#f87171","#60a5fa","#34d399","#a78bfa","#fb923c"].map((c, i) => (
            <div key={i} style={{
              ...
              border: "2px solid #0a1a2e", marginLeft: i > 0 ? -6 : 0,
            }} />
          ))}
        </div>
      </div>
          ...
          <div key={m.name} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "8px 10px" }}>
            ...
            {m.tasks.slice(0, 5).map((t, i) => (
              <div key={i} style={{
                ...
                borderBottom: i < m.tasks.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none",
              }}>
                {i + 1}. {t}
              </div>
            ))}
          ...
          <div style={{ display: "flex", gap: -4 }}>
            {[1,2,3].map((_, j) => (
              <div key={j} style={{
                ...
                border: "2px solid #160830", marginLeft: j > 0 ? -5 : 0,
              }} />
            ))}
          </div>
        </div>
  ...
  return (
    <>
        ...
        <div style={{ display: "flex", gap: 2 }}>
          {[Plus, ChevronDown].map((Icon, i) => (
            <button key={i} type="button" style={{
              width: 24, height: 24, borderRadius: 5, border: 0, background: "none",
              cursor: "pointer", color: "#606068", display: "flex", alignItems: "center", justifyContent: "center",
            }}
      ...
      <div style={{ padding: "8px 6px" }}>
        {[
          { label: "Todas as equipes", active: true, badge: undefined, icon: (
          ...
          <button key={item.label} type="button" style={{
            ...
            background: item.active ? "#202022" : "none",
            color: item.active ? "#e4e4e4" : "#888892", fontSize: 13, fontWeight: item.active ? 500 : 400,
          }}
...
export default function TeamsPage() {
  ...
  /* slides visíveis: anterior (cortado), atual, próximo, próximo+1 (cortado) */
  const getVisible = () => {
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden", background: "#111111" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        {/* corpo: texto à esquerda + carousel à direita — centralizado verticalmente */}
        <div style={{ flex: 1, display: "flex", alignItems: "center", overflow: "hidden", padding: "0 0 0 48px" }}>
          ...
          <div style={{ flexShrink: 0, width: 340, paddingRight: 40 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <button type="button" style={{
                height: 36, padding: "0 4px", borderRadius: 7,
          ...
          {/* carousel — ocupa o resto, overflow hidden */}
          <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
            {/* faixa de cards — altura fixa, não estica a tela */}
            <div style={{
              display: "flex", gap: 14,
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            {SLIDES.map((_, i) => (
              <button key={i} type="button" onClick={() => setSlide(i)} style={{
                ...
                background: i === slide ? "#c4c4c4" : "#2a2a2a",
                transition: "all .2s",
              }} />
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {[
              { icon: ChevronLeft,  onClick: () => setSlide(s => (s - 1 + total) % total) },
              { icon: ChevronRight, onClick: () => setSlide(s => (s + 1) % total) },
            ].map(({ icon: Icon, onClick }, i) => (
              <button key={i} type="button" onClick={onClick} style={{
                width: 28, height: 28, borderRadius: "50%", border: "1px solid #2a2a2a",
