  ...
  return (
    <div style={{ width: "100%", height: "100%", background: "linear-gradient(160deg,#0d2137 0%,#0a1a2e 100%)", borderRadius: 10, padding: "14px 16px", overflow: "hidden" }}>
      <div style={{ display: "flex", gap: 0, marginBottom: 10 }}>
        {["","7","8","9","10","11"].map((n, i) => (
          <div key={i} style={{ flex: i === 0 ? "0 0 60px" : 1, textAlign: "center", fontSize: 11, color: "var(--muted-foreground)", fontWeight: 600 }}>{n}</div>
        ))}
      </div>
      {[
      ...
      <div style={{ marginTop: 12, borderTop: "1px solid var(--border)", paddingTop: 10 }}>
        <div style={{ fontSize: 10, color: "var(--muted-foreground)", marginBottom: 6 }}>Members</div>
      ...
      <div style={{ marginTop: 10 }}>
        <div style={{ fontSize: 10, color: "var(--muted-foreground)", marginBottom: 6 }}>Members online</div>
  ...
  return (
    <div style={{ width: "100%", height: "100%", background: "linear-gradient(160deg,#1a1040 0%,#120c30 100%)", borderRadius: 10, padding: "12px", overflow: "hidden" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {members.map((m) => (
          <div key={m.name} style={{ background: "var(--border)", borderRadius: 8, padding: "8px 10px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
              <div style={{ width: 22, height: 22, borderRadius: "50%", background: m.color, flexShrink: 0 }} />
              <div>
                <p style={{ fontSize: 10, fontWeight: 700, color: "var(--foreground)" }}>{m.name}</p>
                <p style={{ fontSize: 9, color: "var(--muted-foreground)" }}>{m.role}</p>
              </div>
            </div>
            {m.tasks.slice(0, 5).map((t, i) => (
              <div key={i} style={{ fontSize: 9, color: "var(--muted-foreground)", padding: "2px 0", borderBottom: i < m.tasks.length - 1 ? "1px solid var(--border)" : "none" }}>
                {i + 1}. {t}
              </div>
            ))}
          </div>
...
function PreviewPriorities() {
  return (
    <div style={{ width: "100%", height: "100%", background: "linear-gradient(160deg,#1e0a3c 0%,#160830 100%)", borderRadius: 10, padding: "12px", overflow: "hidden" }}>
        ...
        <div key={i} style={{ marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 10, color: "var(--foreground)", fontWeight: 600 }}>{g.label}</span>
            </div>
            <span style={{ fontSize: 9, color: "var(--muted-foreground)" }}>{g.count}</span>
          </div>
          <div style={{ display: "flex", gap: -4 }}>
            {[1,2,3].map((_, j) => (
      ...
      <div style={{ marginTop: 8, background: "var(--border)", borderRadius: 6, padding: "6px 8px" }}>
        <div style={{ fontSize: 9, color: "var(--muted-foreground)", marginBottom: 4 }}>Analytics</div>
...
function EmptyState({ onCreateTeam }: { onCreateTeam: () => void }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {/* topbar */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", height: 52, borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        <h1 style={{ fontSize: 16, fontWeight: 700, color: "var(--foreground)" }}>Todas as equipes</h1>
        <button type="button" onClick={onCreateTeam} style={{ height: 32, padding: "0 16px", borderRadius: 7, background: "var(--primary)", border: "none", cursor: "pointer", color: "var(--primary-foreground)", fontSize: 13, fontWeight: 600 }}
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", overflow: "hidden", padding: "0 0 0 48px" }}>
        <div style={{ flexShrink: 0, width: 408, paddingRight: 40 }}>
          <h2 style={{ fontSize: 42, fontWeight: 900, color: "var(--foreground)", lineHeight: 1.08, marginBottom: 18, letterSpacing: "-0.02em", /* text on light avatar bg */ }}>
            {SLIDES[slide].title}
          </h2>
          <p style={{ fontSize: 14, color: "var(--muted-foreground)", lineHeight: 1.65, marginBottom: 28 }}>
            {SLIDES[slide].desc}
          </p>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button type="button" onClick={onCreateTeam} style={{ height: 36, padding: "0 20px", borderRadius: 7, background: "var(--primary)", border: "none", cursor: "pointer", color: "var(--primary-foreground)", fontSize: 13, fontWeight: 700 }}
              onMouseEnter={e => { e.currentTarget.style.filter = "brightness(1.1)"; }}
              onMouseLeave={e => { e.currentTarget.style.filter = "none"; }}>
              Criar equipe
            </button>
            <button type="button" style={{ height: 36, padding: "0 4px", borderRadius: 7, background: "none", border: "none", cursor: "pointer", color: "var(--muted-foreground)", fontSize: 13, fontWeight: 500 }}
        ...
        <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
          <div style={{ display: "flex", gap: 14, transform: "translateX(-60px)", alignItems: "flex-start" }}>
            {getVisible().map(({ idx, offset }) => {
              ...
              return (
                <div key={`${idx}-${offset}`} style={{ flexShrink: 0, width: 340, height: 420, borderRadius: 14, background: "var(--card)", border: "1px solid var(--border)", overflow: "hidden", opacity: isActive ? 1 : offset === 1 ? 0.85 : 0.4, display: "flex", flexDirection: "column" }}>
                  <div style={{ flex: 1, padding: 12, minHeight: 0, overflow: "hidden" }}>{s.preview}</div>
                  {s.featureTitle && (
                    <div style={{ padding: "10px 16px 16px", flexShrink: 0, borderTop: "1px solid var(--border)" }}>
                      <p style={{ fontSize: 13, fontWeight: 700, color: "var(--foreground)", marginBottom: isActive ? 4 : 0 }}>{s.featureTitle}</p>
                      {isActive && <p style={{ fontSize: 11, color: "var(--muted-foreground)", lineHeight: 1.5 }}>{s.featureDesc}</p>}
                    </div>
                  )}
                </div>
              );
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, height: 56, flexShrink: 0 }}>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {SLIDES.map((_, i) => (
            <button key={i} type="button" onClick={() => setSlide(i)} style={{ width: i === slide ? 22 : 8, height: 8, borderRadius: 4, border: 0, cursor: "pointer", padding: 0, background: i === slide ? "var(--foreground)" : "var(--accent)", transition: "all .2s" }} />
          ))}
        </div>
        <div style={{ display: "flex", gap: 4 }}>
          {[
            ...
            <button key={i} type="button" onClick={fn} style={{ width: 28, height: 28, borderRadius: "50%", border: "1px solid #2a2a2a", background: "none", cursor: "pointer", color: "var(--muted-foreground)", display: "flex", alignItems: "center", justifyContent: "center" }}
  ...
  return (
    ...
    >
      ...
      <div style={{ height: 110, background: "linear-gradient(160deg,#1c1c2e 0%,#111118 100%)", position: "relative", overflow: "hidden", borderRadius: "10px 10px 0 0" }}>
        <div style={{ position: "absolute", top: 22, left: 16, right: 16, height: 6, borderRadius: 3, background: "var(--border)" }} />
        <div style={{ position: "absolute", top: 36, left: 16, right: 40, height: 6, borderRadius: 3, background: "var(--border)" }} />
        <div style={{ position: "absolute", top: 50, left: 16, right: 28, height: 6, borderRadius: 3, background: "var(--border)" }} />
          ...
          >
            {[
              ...
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.background = item.danger ? "rgba(248,113,113,0.08)" : "var(--border)"; }}
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 9, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: 13, fontWeight: 600, color: "var(--foreground)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {team.nome}
              </p>
          ...
          <button
            ...
            style={{ width: 26, height: 26, borderRadius: 6, border: "1px solid var(--border)", background: "none", cursor: "pointer", color: "var(--muted-foreground)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "color .15s, border-color .15s" }}
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", height: 52, borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        <h1 style={{ fontSize: 15, fontWeight: 700, color: "var(--foreground)" }}>Todas as equipes</h1>
        <button type="button" onClick={onCreateTeam} style={{ height: 30, padding: "0 14px", borderRadius: 7, background: "var(--primary)", border: "none", cursor: "pointer", color: "var(--primary-foreground)", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 5 }}
          onMouseEnter={e => { e.currentTarget.style.filter = "brightness(1.1)"; }}
          onMouseLeave={e => { e.currentTarget.style.filter = "none"; }}>
          Criar equipe
        </button>
      ...
      {/* barra de filtros */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", height: 44, borderBottom: "1px solid var(--border)", flexShrink: 0, gap: 8 }}>
        {/* filtros esquerda */}
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          {filters.map(f => (
            <button key={f.id} type="button"
              ...
              style={{
                ...
                borderColor: activeFilter === f.id ? "var(--border)" : "var(--border)",
                background: activeFilter === f.id ? "var(--border)" : "none",
                cursor: "pointer", color: activeFilter === f.id ? "var(--foreground)" : "var(--muted-foreground)",
                fontSize: 12, fontWeight: 500, display: "flex", alignItems: "center", gap: 4,
                transition: "all .15s",
              }}
              onMouseEnter={e => { if (activeFilter !== f.id) { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.style.color = "var(--foreground)"; }}}
              onMouseLeave={e => { if (activeFilter !== f.id) { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.style.color = "var(--muted-foreground)"; }}}
            >
              {f.label}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          {searchOpen ? (
            <input
              ...
              style={{ height: 28, padding: "0 10px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--border)", color: "var(--foreground)", fontSize: 12, outline: "none", width: 180 }}
            />
          ) : (
            <button type="button" onClick={() => setSearchOpen(true)} style={{ width: 28, height: 28, borderRadius: 6, border: "1px solid var(--border)", background: "none", cursor: "pointer", color: "var(--muted-foreground)", display: "flex", alignItems: "center", justifyContent: "center" }}
              onMouseEnter={e => { e.currentTarget.style.color = "var(--foreground)"; }}
              onMouseLeave={e => { e.currentTarget.style.color = "var(--muted-foreground)"; }}>
          ...
          {[
            ...
            <button key={mode} type="button" onClick={() => setViewMode(mode)} style={{ width: 28, height: 28, borderRadius: 6, border: "1px solid", borderColor: viewMode === mode ? "var(--border)" : "var(--border)", background: viewMode === mode ? "var(--border)" : "none", cursor: "pointer", color: viewMode === mode ? "var(--foreground)" : "var(--muted-foreground)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 20px" }}>
        {filtered.length === 0 && search ? (
          <p style={{ color: "var(--muted-foreground)", fontSize: 13 }}>Nenhuma equipe encontrada para "{search}".</p>
        ) : (
...
const TEAM_ICONS: { name: string; path: string }[] = [
  ...
  { name: "box",         path: "M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" },
  ...
  { name: "git-branch",  path: "M6 3v12M18 9a3 3 0 1 0 0-6 3 3 0 0 0 0 6zM6 21a3 3 0 1 0 0-6 3 3 0 0 0 0 6zM18 9a9 9 0 0 1-9 9" },
  ...
  { name: "phone",       path: "M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" },
  ...
  { name: "printer",     path: "M6 9V2h12v7M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2M6 14h12v8H6z" },
  { name: "wifi",        path: "M5 12.55a11 11 0 0 1 14.08 0M1.42 9a16 16 0 0 1 21.16 0M8.53 16.11a6 6 0 0 1 6.95 0M12 20h.01" },
  ...
  { name: "eye",         path: "M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8zM12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z" },
  { name: "filter",      path: "M22 3H2l8 9.46V19l4 2v-8.54L22 3z" },
  { name: "copy",        path: "M20 9H11a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2v-9a2 2 0 0 0-2-2zM5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" },
  { name: "upload",      path: "M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12" },
  { name: "download",    path: "M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3" },
  { name: "maximize",    path: "M8 3H5a2 2 0 0 0-2 2v3M21 8V5a2 2 0 0 0-2-2h-3M3 16v3a2 2 0 0 0 2 2h3M16 21h3a2 2 0 0 0 2-2v-3" },
  ...
  { name: "info",        path: "M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20zM12 16v-4M12 8h.01" },
  { name: "alert",       path: "M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0zM12 9v4M12 17h.01" },
  { name: "help",        path: "M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20zM9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3M12 17h.01" },
  ...
  { name: "clock",       path: "M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20zM12 6v6l4 2" },
  { name: "refresh",     path: "M23 4v6h-6M1 20v-6h6M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" },
  ...
  { name: "paper-clip",  path: "M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48" },
  ...
  { name: "video",       path: "M22.54 6.42a2.78 2.78 0 0 0-1.95-1.95C18.88 4 12 4 12 4s-6.88 0-8.59.47a2.78 2.78 0 0 0-1.95 1.95A29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58 2.78 2.78 0 0 0 1.95 1.95C5.12 20 12 20 12 20s6.88 0 8.59-.47a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58zM9.75 15.02V8.98L15.5 12l-5.75 3.02z" },
];
...
function CreateTeamModal({ onClose, onCreate }: { onClose: () => void; onCreate: (p: CreateTeamPayloadLocal) => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.55)" }}
      ...
      <div style={{ width: 460, borderRadius: 12, background: "var(--card)", border: "1px solid var(--border)", padding: "28px 28px 24px", boxShadow: "0 24px 64px rgba(0,0,0,0.6)" }}
        onClick={e => e.stopPropagation()}>
        <h2 style={{ fontSize: 16, fontWeight: 700, color: "var(--foreground)", marginBottom: 6 }}>Criar equipe</h2>
        ...
        <form onSubmit={handleSubmit}>
          <p style={{ fontSize: 12, color: "var(--muted-foreground)", fontWeight: 500, marginBottom: 8 }}>Ícone e nome</p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: panelOpen ? 0 : 20 }}>
            <button type="button"
              ...
              style={{
                ...
                background: color,
                border: panelOpen ? "2px solid var(--border)" : "2px solid transparent",
            ...
            <input
              ...
              style={{ flex: 1, height: 42, padding: "0 14px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--border)", color: "var(--foreground)", fontSize: 13, outline: "none", boxSizing: "border-box" }}
          ...
          {panelOpen && (
            <div style={{ marginTop: 2, marginBottom: 20, borderRadius: 8, border: "1px solid var(--border)", background: "var(--card)", overflow: "visible" }}>
              {/* cabeçalho fixo da aba ícone */}
              <div style={{ display: "flex", borderBottom: "1px solid var(--border)", padding: "0 10px" }}>
                <div style={{ height: 38, display: "flex", alignItems: "center", fontSize: 13, fontWeight: 500, color: "var(--foreground)", borderBottom: "2px solid #e4e4e4", paddingBottom: 0 }}>
                  Ícone
                </div>
              </div>
              ...
              <div style={{ padding: "10px 10px 12px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10, position: "relative" }}>
                  <input
                    ...
                    style={{ flex: 1, height: 30, padding: "0 10px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--border)", color: "var(--foreground)", fontSize: 12, outline: "none" }}
                  />
                  ...
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    <button type="button"
                      ...
                      style={{ width: 24, height: 24, borderRadius: "50%", border: "2px solid var(--border)", background: color, cursor: "pointer", display: "block" }} />
                  ...
                  {filteredIcons.map(opt => (
                    <button key={opt.name} type="button"
                      ...
                      style={{
                        ...
                        background: icon === opt.name ? "var(--border)" : "none",
                        cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
                        color: icon === opt.name ? "var(--foreground)" : "var(--muted-foreground)", transition: "background .1s, color .1s",
                      }}
                      onMouseEnter={e => { if (icon !== opt.name) { e.currentTarget.style.background = "var(--border)"; e.currentTarget.style.color = "var(--foreground)"; }}}
                      onMouseLeave={e => { if (icon !== opt.name) { e.currentTarget.style.background = "none"; e.currentTarget.style.color = "var(--muted-foreground)"; }}}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10 }}>
                  <button type="button" onClick={() => setPanelOpen(false)}
                    style={{ height: 28, padding: "0 14px", borderRadius: 6, border: "none", background: "var(--border)", cursor: "pointer", color: "var(--foreground)", fontSize: 12, fontWeight: 500 }}
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
            ...
            <button type="submit" disabled={!nome.trim()}
              style={{ height: 34, padding: "0 20px", borderRadius: 7, border: "none", background: nome.trim() ? "var(--foreground)" : "var(--accent)", cursor: nome.trim() ? "pointer" : "not-allowed", color: nome.trim() ? "var(--primary-foreground)" : "var(--muted-foreground)", fontSize: 13, fontWeight: 600, transition: "all .15s" }}>
              Criar equipe
            </button>
          </div>
        </form>
...
function EditTeamModal({ team, onClose, onSave }: { team: TeamLocal; onClose: () => void; onSave: (id: string, nome: string) => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.55)" }}
      ...
      <div style={{ width: 420, borderRadius: 12, background: "var(--card)", border: "1px solid var(--border)", padding: "28px 28px 24px", boxShadow: "0 24px 64px rgba(0,0,0,0.6)" }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, color: "var(--foreground)", marginBottom: 6 }}>Editar equipe</h2>
        <p style={{ fontSize: 13, color: "var(--muted-foreground)", marginBottom: 20, lineHeight: 1.5 }}>
          Altere o nome da equipe.
        </p>
        <form onSubmit={handleSubmit}>
          <label style={{ display: "block", fontSize: 12, color: "var(--muted-foreground)", marginBottom: 6 }}>Nome da equipe</label>
          <input
            autoFocus
            value={nome}
            onChange={e => setNome(e.target.value)}
            placeholder="Nome da equipe..."
            style={{ width: "100%", height: 38, padding: "0 12px", borderRadius: 7, border: "1px solid var(--border)", background: "var(--border)", color: "var(--foreground)", fontSize: 13, outline: "none", boxSizing: "border-box" }}
          />
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
            <button type="button" onClick={onClose} style={{ height: 34, padding: "0 16px", borderRadius: 7, border: "1px solid var(--border)", background: "none", cursor: "pointer", color: "var(--muted-foreground)", fontSize: 13 }}
              onMouseEnter={e => { e.currentTarget.style.color = "var(--foreground)"; }}
              onMouseLeave={e => { e.currentTarget.style.color = "var(--muted-foreground)"; }}>
              Cancelar
            </button>
            <button type="submit" disabled={!nome.trim()} style={{ height: 34, padding: "0 20px", borderRadius: 7, border: "none", background: nome.trim() ? "var(--foreground)" : "var(--accent)", cursor: nome.trim() ? "pointer" : "not-allowed", color: nome.trim() ? "var(--primary-foreground)" : "var(--muted-foreground)", fontSize: 13, fontWeight: 600, transition: "all .15s" }}>
              Salvar
            </button>
          </div>
        </form>
      ...
      <div style={{ width: 420, borderRadius: 12, background: "var(--card)", border: "1px solid var(--border)", padding: "28px 28px 24px", boxShadow: "0 24px 64px rgba(0,0,0,0.6)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <button type="button" onClick={onClose} disabled={isDeleting}
            style={{ height: 34, padding: "0 16px", borderRadius: 7, border: "1px solid var(--border)", background: "none", cursor: isDeleting ? "not-allowed" : "pointer", color: "var(--muted-foreground)", fontSize: 13, opacity: isDeleting ? 0.5 : 1 }}
            onMouseEnter={e => { if (!isDeleting) e.currentTarget.style.color = "var(--foreground)"; }}
            onMouseLeave={e => { e.currentTarget.style.color = "var(--muted-foreground)"; }}>
            Cancelar
          </button>
...
function AllPeopleView({ pessoas }: { pessoas: PersonMock[] }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {/* topbar */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", height: 52, borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        <h1 style={{ fontSize: 15, fontWeight: 700, color: "var(--foreground)" }}>Todas as pessoas</h1>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button type="button" style={{ height: 30, padding: "0 14px", borderRadius: 7, border: "1px solid var(--border)", background: "none", cursor: "pointer", color: "var(--foreground)", fontSize: 13, fontWeight: 500, display: "flex", alignItems: "center", gap: 6 }}
          ...
          <button type="button" style={{ height: 30, padding: "0 14px", borderRadius: 7, border: "none", background: "var(--primary)", cursor: "pointer", color: "var(--primary-foreground)", fontSize: 13, fontWeight: 600, transition: "filter 120ms" }}
            onMouseEnter={e => { e.currentTarget.style.filter = "brightness(1.1)"; }}
            onMouseLeave={e => { e.currentTarget.style.filter = "none"; }}>
            Convidar
          </button>
      ...
      {/* barra de filtros */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", height: 44, borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          {filters.map(f => (
            <button key={f.id} type="button"
              onClick={() => setActiveFilter(activeFilter === f.id ? null : f.id)}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          {searchOpen ? (
            ...
            <button type="button" onClick={() => setSearchOpen(true)}
              style={{ width: 28, height: 28, borderRadius: 6, border: "1px solid var(--border)", background: "none", cursor: "pointer", color: "var(--muted-foreground)", display: "flex", alignItems: "center", justifyContent: "center" }}
              onMouseEnter={e => { e.currentTarget.style.color = "var(--foreground)"; }}
              onMouseLeave={e => { e.currentTarget.style.color = "var(--muted-foreground)"; }}>
              <Search size={13} strokeWidth={1.8} />
            </button>
          ...
          {([
            ...
            <button key={mode} type="button" onClick={() => setViewMode(mode)}
              style={{ width: 28, height: 28, borderRadius: 6, border: "1px solid", borderColor: viewMode === mode ? "var(--border)" : "var(--border)", background: viewMode === mode ? "var(--border)" : "none", cursor: "pointer", color: viewMode === mode ? "var(--foreground)" : "var(--muted-foreground)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Icon size={13} strokeWidth={1.8} />
            </button>
          ))}
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
        {filtered.length === 0 && search ? (
          <p style={{ color: "var(--muted-foreground)", fontSize: 13 }}>Nenhuma pessoa encontrada para "{search}".</p>
        ) : (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 14 }}>
            {filtered.map(p => (
              <div key={p.id} style={{ width: 148, borderRadius: 10, background: "var(--accent)", border: "1px solid var(--border)", overflow: "hidden", cursor: "pointer", transition: "border-color .15s" }}
                onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "var(--border)"; }}
                onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "var(--border)"; }}>
                {/* avatar grande */}
                <div style={{ height: 148, background: "var(--muted)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                ...
                <div style={{ padding: "10px 12px 12px", display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: "var(--foreground)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {p.nome}
                  </span>
                  {p.online && (
      ...
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 12px", height: 44, borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 2 }}>
          {[Plus, ChevronDown].map((Icon, i) => (
            <button key={i} type="button" style={{ width: 24, height: 24, borderRadius: 5, border: 0, background: "none", cursor: "pointer", color: "var(--muted-foreground)", display: "flex", alignItems: "center", justifyContent: "center" }}
      ...
      <div style={{ padding: "8px 6px" }}>
        {navItems.map((item) => {
          ...
          return (
            <button key={item.label} type="button"
              ...
              style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", height: 34, padding: "0 8px", borderRadius: 5, border: 0, cursor: "pointer", textAlign: "left", background: isActive ? "var(--accent)" : "none", color: isActive ? "var(--foreground)" : "var(--muted-foreground)", fontSize: 13, fontWeight: isActive ? 500 : 400 }}
              onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = "var(--accent)"; }}
              onMouseLeave={e => { if (!isActive) e.currentTarget.style.background = "none"; }}>
              <span style={{ color: isActive ? "var(--foreground)" : "var(--muted-foreground)", flexShrink: 0 }}>{item.icon}</span>
              <span style={{ flex: 1 }}>{item.label}</span>
              {item.badge && (
                <span style={{ fontSize: 11, color: "var(--muted-foreground)", background: "var(--accent)", borderRadius: 4, padding: "0 5px", lineHeight: "18px" }}>{item.badge}</span>
              )}
            </button>
          );
        })}

        <p style={{ fontSize: 11, fontWeight: 600, color: "var(--muted-foreground)", textTransform: "uppercase", letterSpacing: "0.06em", padding: "12px 8px 6px" }}>
          Minhas equipes
        </p>
        ...
        {teams.length === 0 ? (
          <div style={{ margin: "0 2px", padding: "16px 10px", borderRadius: 8, border: "1px dashed #282828", textAlign: "center" }}>
            ...
            <p style={{ fontSize: 11, color: "var(--muted-foreground)", lineHeight: 1.6 }}>Depois de entrar ou criar uma<br />equipe, você a verá aqui</p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {teams.map(t => (
              <button key={t.id} type="button" onClick={() => onTeamClick?.(t.id)} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", height: 32, padding: "0 8px", borderRadius: 5, border: 0, cursor: "pointer", background: "none", color: "var(--muted-foreground)", fontSize: 13 }}
